...
QVariant UpnpAlbumModel::internalDataTrack(const QModelIndex &index, int role, DidlParser *currentParser) const
{
    ...
    case ColumnsRoles::TitleRole:
        return currentParser->newMusicTracks()[index.row()].title();
    case ColumnsRoles::DurationRole:
        if (currentParser->newMusicTracks()[index.row()].duration().hour() == 0) {
            return currentParser->newMusicTracks()[index.row()].duration().toString(QStringLiteral("mm:ss"));
        } else {
            return currentParser->newMusicTracks()[index.row()].duration().toString();
        }
    case ColumnsRoles::CreatorRole:
        return currentParser->newMusicTracks()[index.row()].artist();
    case ColumnsRoles::ArtistRole:
        return currentParser->newMusicTracks()[index.row()].artist();
    case ColumnsRoles::AlbumRole:
        return currentParser->newMusicTracks()[index.row()].albumName();
    case ColumnsRoles::TrackNumberRole:
        return currentParser->newMusicTracks()[index.row()].trackNumber();
    case ColumnsRoles::RatingRole:
        return 0;
    case ColumnsRoles::ImageRole:
        return data(index.parent(), role);
    case ColumnsRoles::ResourceRole:
        return currentParser->newMusicTracks()[index.row()].resourceURI();
    case ColumnsRoles::ItemClassRole:
        return {};
    case ColumnsRoles::CountRole:
        return {};
    case ColumnsRoles::IdRole:
        return currentParser->newMusicTracks()[index.row()].id();
