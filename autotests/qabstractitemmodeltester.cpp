...
void QAbstractItemModelTesterPrivate::rowsAboutToBeInserted(const QModelIndex &parent, int start, int end)
{
    qCDebug(lcModelTest) << "rowsAboutToBeInserted"
                         ...
                         << "parent data=" << (parent.isValid() ? model->data(parent).toString() : QString{})
                         << "current count of parent=" << model->rowCount(parent)
                         << "last before insertion=" << (start > 1 ? model->index(start - 1, 0, parent) : QModelIndex{})
                         << (start > 1 ? model->data(model->index(start - 1, 0, parent)) : QModelIndex{});
...
void QAbstractItemModelTesterPrivate::rowsInserted(const QModelIndex &parent, int start, int end)
{
    ...
    if (end + 1 < model->rowCount(c.parent)) {
        if (c.next != model->data(model->index(end + 1, 0, c.parent))) {
            qCDebug(lcModelTest) << start << end;
            for (int i = 0; i < model->rowCount(); ++i)
                qCDebug(lcModelTest) << model->index(i, 0).data().toString();
            qCDebug(lcModelTest) << c.next << model->data(model->index(end + 1, 0, c.parent));
        }
...
void QAbstractItemModelTesterPrivate::rowsAboutToBeRemoved(const QModelIndex &parent, int start, int end)
{
    qCDebug(lcModelTest) << "rowsAboutToBeRemoved"
                         ...
                         << "parent data=" << (parent.isValid() ? model->data(parent).toString() : QString{})
                         << "current count of parent=" << model->rowCount(parent)
                         << "last before removal=" << (start > 1 ? model->index(start - 1, 0, parent) : QModelIndex{})
                         << (start > 1 ? model->data(model->index(start - 1, 0, parent)) : QVariant{});

    Changing c;
    c.parent = parent;
    c.oldSize = model->rowCount(parent);
    c.last = (start > 1 ? model->data(model->index(start - 1, 0, parent)) : QVariant{});
    c.next = (end < model->rowCount(parent) - 1 ? model->data(model->index(end + 1, 0, parent)) : QVariant{});
    remove.push(c);
}
...
void QAbstractItemModelTesterPrivate::rowsRemoved(const QModelIndex &parent, int start, int end)
{
    ...
    MODELTESTER_COMPARE(parent, c.parent);
    MODELTESTER_COMPARE(model->rowCount(parent), c.oldSize - (end - start + 1));
    MODELTESTER_COMPARE((start > 1 ? model->data(model->index(start - 1, 0, c.parent)) : QVariant{}), c.last);
    MODELTESTER_COMPARE((start < model->rowCount(parent) ? model->data(model->index(start, 0, c.parent)) : QVariant{}), c.next);
}
